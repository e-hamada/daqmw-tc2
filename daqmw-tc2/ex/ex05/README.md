...
ヒストグラムの大きさ(=TCanvasの大きさ）は

    m_canvas = new TCanvas("c1", "histos", 0, 0, 600, 400);

この部分で定義されている。x方向の長さが600、y方向の長さが400となっており、これらの値をコンフィグレーションファイルのパラメータから変えられるようにする。daq_configure関数でもdaq_start関数でも読み書きできる変数を扱うためには、ヘッダファイルにその変数を定義する必要がある。
