...
int SampleMonitor::parse_params(::NVList* list)
{
    ...
    for (int i = 0; i < len; i+=2) {
        ...
        if (sname == "Canvas_x_length") {
            if (m_debug) {
                std::cerr << "monitor update rate: " << svalue << std::endl;
            }
            char *offset;
            m_Canvas_x_length = (int)strtol(svalue.c_str(), &offset, 10);
        }
        if (sname == "Canvas_y_length") {
            if (m_debug) {
                std::cerr << "monitor update rate: " << svalue << std::endl;
            }
            char *offset;
            m_Canvas_y_length = (int)strtol(svalue.c_str(), &offset, 10);
        }
    }
...
int SampleMonitor::daq_start()
{
    ...
    m_canvas = new TCanvas("c1", "histos", 0, 0, m_Canvas_x_length, m_Canvas_y_length);
