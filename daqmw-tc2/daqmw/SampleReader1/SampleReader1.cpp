...
static const char* samplereader1_spec[] =
{
    "implementation_id", "SampleReader1",
    "type_name",         "SampleReader1",
    "description",       "SampleReader1 component",
    "version",           "1.0",
...
SampleReader1::SampleReader1(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_OutPort("samplereader1_out", m_out_data),
      m_sock(0),
      m_recv_byte_size(0),
      ...
      m_debug(false)
{
    ...
    // Set OutPort buffers
    registerOutPort("samplereader1_out", m_OutPort);

    init_command_port();
    init_state_table();
    set_comp_name("SAMPLEREADER1");
}

SampleReader1::~SampleReader1()
{
}

RTC::ReturnCode_t SampleReader1::onInitialize()
{
    if (m_debug) {
        std::cerr << "SampleReader1::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t SampleReader1::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int SampleReader1::daq_dummy()
{
    return 0;
}

int SampleReader1::daq_configure()
{
    std::cerr << "*** SampleReader1::configure" << std::endl;
...
int SampleReader1::parse_params(::NVList* list)
{
...
int SampleReader1::daq_unconfigure()
{
    std::cerr << "*** SampleReader1::unconfigure" << std::endl;

    return 0;
}

int SampleReader1::daq_start()
{
    std::cerr << "*** SampleReader1::start" << std::endl;
...
int SampleReader1::daq_stop()
{
    std::cerr << "*** SampleReader1::stop" << std::endl;
...
int SampleReader1::daq_pause()
{
    std::cerr << "*** SampleReader1::pause" << std::endl;

    return 0;
}

int SampleReader1::daq_resume()
{
    std::cerr << "*** SampleReader1::resume" << std::endl;

    return 0;
}

int SampleReader1::read_data_from_detectors()
{
...
int SampleReader1::set_data(unsigned int data_byte_size)
{
    ...
    unsigned short myPort = (unsigned short)m_srcPort;
    memcpy(&header[2], &(myPort), sizeof(unsigned short));
...
int SampleReader1::write_OutPort()
{
...
int SampleReader1::daq_run()
{
    if (m_debug) {
        std::cerr << "*** SampleReader1::run" << std::endl;
    }
...
extern "C"
{
    void SampleReader1Init(RTC::Manager* manager)
    {
        RTC::Properties profile(samplereader1_spec);
        manager->registerFactory(profile,
                    RTC::Create<SampleReader1>,
                    RTC::Delete<SampleReader1>);
    }
};
