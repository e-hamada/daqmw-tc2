...
static const char* mylogger_spec[] = {
    "implementation_id", "RawDataLogger",
    "type_name",         "RawDataLogger",
...
RawDataLogger::RawDataLogger(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_InPort("rawdatalogger_in", m_in_data),
      ...
      m_debug(false)
{
    ...
    registerInPort("rawdatalogger_in", m_InPort);

    init_command_port();
    init_state_table();
    set_comp_name("RawDataLogger");
}
...
RTC::ReturnCode_t RawDataLogger::onInitialize()
{
    if (m_debug) {
        std::cerr << "RawDataLogger::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t RawDataLogger::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int RawDataLogger::daq_dummy()
{
    return 0;
}

int RawDataLogger::daq_configure()
{
...
int RawDataLogger::parse_params(::NVList* list)
{
    int ret = 0;
    ...
    for (int i = 0; i < length; i += 2) {
        ...
        if (sname == "isLogging") {
            ...
            if (svalue == "yes") {
                ...
                isExistParamLogging = true;
                std::cerr << "RawDataLogger: Save to file: true\n";
            }
            else if (svalue == "no") {
                m_isDataLogging = false;
                isExistParamLogging = true;
                std::cerr << "RawDataLogger: Save to file: false\n";
            }
        }
...
int RawDataLogger::daq_unconfigure()
{
    std::cerr << "*** RawDataLogger::unconfigure" << std::endl;
    if (m_isDataLogging) {
        delete fileUtils;
        if (m_debug) {
...
int RawDataLogger::daq_start()
{
    std::cerr << "*** RawDataLogger::start" << std::endl;
    ...
    if (m_isDataLogging) {
        ...
        if (ret < 0) {
            std::cerr << "### ERROR: RawDataLogger: open file failed\n";
            fatal_error_report(CANNOT_OPEN_FILE);
        }
        else {
            std::cerr << "*** RawDataLogger: file open succeed\n";
            m_filesOpened = true;
        }
    }
    return 0;
...
int RawDataLogger::daq_stop()
{
    std::cerr << "*** RawDataLogger::stop" << std::endl;

    if (m_isDataLogging && m_filesOpened) {
        if (m_debug) {
            std::cerr << "RawDataLogger::stop: close files \n";
        }
...
int RawDataLogger::daq_pause()
{
    std::cerr << "*** RawDataLogger::pause" << std::endl;
    return 0;
}

int RawDataLogger::daq_resume()
{
    std::cerr << "*** RawDataLogger::resume" << std::endl;
    return 0;
}

int RawDataLogger::reset_InPort()
{
    bool ret = true;
    while (ret) {
        ret = m_InPort.read();
    ...
    if (m_debug) {
        std::cerr << "*** RawDataLogger::InPort flushed\n";
    }
    return 0;
}

void RawDataLogger::toLower(std::basic_string<char>& s)
{
...
int RawDataLogger::daq_run()
{
    ...
    if (m_isDataLogging) {
        ...
        if (ret < 0) {
            std::cerr << "### RawDataLogger: ERROR occured at data saving\n";
    ...
    if (m_debug) {
        ...
        if (seq_num % m_update_rate == 0) {
            std::cerr << "RawDataLogger: loop = " << seq_num << std::endl;
            std::cerr << "\033[A\r";
        }
    }
...
extern "C"
{
    void RawDataLoggerInit(RTC::Manager* manager)
    {
        RTC::Properties profile(mylogger_spec);
        manager->registerFactory(profile,
                                 RTC::Create<RawDataLogger>,
                                 RTC::Delete<RawDataLogger>);
    }
};
